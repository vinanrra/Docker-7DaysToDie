...
if [ "${VERSION}" == 'stable' ] || [ "${VERSION}" == 'public' ] || [ "${VERSION}" == 'latest_experimental' ] || [ "${VERSION::7}" == 'alpha21' ]; then
    echo "BepInEx isn't supported with Alpha 21 or above"
    exit 0
fi

# Get latest version
DL_LINK=$(curl -fsSL https://api.github.com/repos/BepInEx/BepInEx/releases/latest | grep -o -E "https://github.com/BepInEx/BepInEx/releases/download/(.*)/BepInEx_unix_(.*).zip")

downloadRelease() {
    curl "$DL_LINK" -fsSL -o BepInEx.zip
}
...
if grep -q "7DaysToDieServer.x86_64" "$BEPINEX_SH"; then
    echo "[BepInEx] Skipping executable_name changes, already replaced"
else
...
if grep -q "sdtdserver.xml" "$BEPINEX_SH"; then
...
echo "[BepInEx] Applying executable permissions"
